...
import { dataAboutPage } from "@/data"; // de aqui traemos los datos de mi trayectoria profesional
          ...
          {dataAboutPage.map((data) => (
            ...
            <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
              ...
              <div className="flex flex-col sm:flex-row items-start mb-1 
                ...
                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-auto p-2 h-6 mb-3 sm:mb-0 text-emerald-600 bg-emerald-100 rounded-full">{data.date}</time>
                <div className="text-xl font-bold text-gray-400">{data.subtitle}</div>
              </div>
              <div className="text-slate-400 max-w-xl">{data.description.map((line, i) => (
                <p key={i}>- {line}</p>
              ))}
              </div>
            </div>
